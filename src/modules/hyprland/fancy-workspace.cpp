...
FancyWorkspace::FancyWorkspace(const Json::Value& workspace_data,
                               FancyWorkspaces& workspace_manager, const Json::Value& clients_data)
  ...
  m_button.signal_button_press_event().connect(sigc::mem_fun(*this, &FancyWorkspace::handleClicked),
                                               false);
...
std::optional<FancyWindowRepr> FancyWorkspace::closeWindow(WindowAddress const& addr) {
...
bool FancyWorkspace::handleClicked(GdkEventButton* bt) const {
...
void FancyWorkspace::initializeWindowMap(const Json::Value& clients_data) {
  m_windowMap.clear();
...
void FancyWorkspace::setActiveWindow(WindowAddress const& addr) {
  ...
  if (activeIdx.has_value() && activeWindowPos != FancyWorkspaces::ActiveWindowPosition::NONE) {
    auto window = std::move(m_windowMap[*activeIdx]);
    m_windowMap.erase(m_windowMap.begin() + *activeIdx);
    if (activeWindowPos == FancyWorkspaces::ActiveWindowPosition::FIRST) {
      m_windowMap.insert(m_windowMap.begin(), std::move(window));
    } else if (activeWindowPos == FancyWorkspaces::ActiveWindowPosition::LAST) {
      m_windowMap.emplace_back(std::move(window));
    }
  }
}

void FancyWorkspace::insertWindow(FancyWindowCreationPayload create_window_payload) {
...
bool FancyWorkspace::onWindowOpened(FancyWindowCreationPayload const& create_window_payload) {
...
std::string& FancyWorkspace::selectIcon(std::map<std::string, std::string>& icons_map) {
...
void FancyWorkspace::update(const std::string& workspace_icon) {
...
void FancyWorkspace::updateWindowIcons() {
  ...
  if (showMode == FancyWorkspaces::ShowWindowIcons::NONE) {
    return;
  }

  // Check if we should show icons based on current-group logic
  if (showMode == FancyWorkspaces::ShowWindowIcons::CURRENT_GROUP) {
  ...
  for (const auto& icon_name : icon_names_ordered) {
    ...
    if (!addresses.empty()) {
      ...
      eventBox->signal_button_press_event().connect(
          [this, firstWindowAddress, icon_name](GdkEventButton* event) -> bool {
...
bool FancyWorkspace::isEmpty() const {
...
void FancyWorkspace::setLabelText(const std::string& text) { m_labelBefore.set_markup(text); }

std::vector<std::string> FancyWorkspace::getWindowClasses() const {
...
std::vector<FancyWorkspace::WindowInfo> FancyWorkspace::getWindows() const {
...
void FancyWorkspace::updateTaskbar(const std::string& workspace_icon) {
  ...
  bool isFirst = true;
  auto processWindow = [&](const FancyWindowRepr& window_repr) {
...
bool FancyWorkspace::handleClick(const GdkEventButton* event_button,
